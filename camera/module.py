...
class Camera():
    ...
    def load_config(self):
        ...
        if camera_config:
            ...
            camera.set(cv2.CAP_PROP_FRAME_WIDTH, camera_config.get("width", 0))
            camera.set(cv2.CAP_PROP_FRAME_HEIGHT, camera_config.get("height", 0))
            camera.set(cv2.CAP_PROP_FPS, camera_config.get("fps", 0))
            camera.set(cv2.CAP_PROP_BRIGHTNESS, camera_config.get("brightness", 0))
            camera.set(cv2.CAP_PROP_CONTRAST, camera_config.get("contrast", 0))
            camera.set(cv2.CAP_PROP_SATURATION, camera_config.get("saturation", 0))
            camera.set(cv2.CAP_PROP_HUE, camera_config.get("hue", 0))
            camera.set(cv2.CAP_PROP_GAIN, camera_config.get("gain", 0))
            camera.set(cv2.CAP_PROP_EXPOSURE, camera_config.get("exposure", 0))
